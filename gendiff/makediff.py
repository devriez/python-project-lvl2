...
def make_diff(file1, file2=None):
    ...
    for key in all_keys:
        if key in file1 and key not in file2:
            result[key] = {'status1': 'deleted', 'body1': make_diff(file1[key])}
        elif key in file2 and key not in file1:
            result[key] = {'status1': 'added', 'body1': make_diff(file2[key])}
        else:
            if file1[key] == file2[key]:
                result[key] = {'status1': 'unchanged', 'body1': make_diff(file1[key])}
            elif (type(file1[key]) != dict) or (type(file2[key]) != dict):
                result[key] = {'status1': 'replaced', 'body1': make_diff(file1[key]), 'body2': make_diff(file2[key])}
            else:
